...
# Box64 crashes in Unity Burst (lib_burst_generated, illegal instruction).
# SteamCMD validate restores the DLL, so hide it on every start.
BURST="$s/VRisingServer_Data/Plugins/x86_64/lib_burst_generated.dll"
if [ -f "$BURST" ]; then
    echo "Disabling Unity Burst native plugin for Box64"
    mv -f "$BURST" "${BURST}.disabled"
fi
printf "1604030\n" > "$s/steam_appid.txt"
printf "steam_appid: "
