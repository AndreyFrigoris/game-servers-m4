...
prune_old_backups() {
  find "$BACKUP_ROOT" -type f -name "conan-backup-*.tar.gz" -mtime "+$BACKUP_RETENTION_DAYS" -delete
}

create_backup() {
  local now archive
  now="$(date +"%Y-%m-%dT%H-%M-%S")"
  archive="$BACKUP_ROOT/conan-backup-$now.tar.gz"
  ...
  # SQLite world — only archive with the server stopped if you can.
  tar -czf "$archive" \
    -C "$PROJECT_ROOT" \
    persistentdata
