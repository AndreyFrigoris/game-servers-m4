...
DATA_DIR="$PROJECT_ROOT/vrising/persistentdata"
...
prune_old_backups() {
  find "$BACKUP_ROOT" -type f -name "vrising-backup-*.tar.gz" -mtime "+$BACKUP_RETENTION_DAYS" -delete
}

create_backup() {
  local now archive
  now="$(date +"%Y-%m-%dT%H-%M-%S")"
  archive="$BACKUP_ROOT/vrising-backup-$now.tar.gz"
